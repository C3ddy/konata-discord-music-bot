...
var shift = new db.table('shiftDatabase')
...
exports.run = async (client, message, args, distube) => {
    ...
    if(shift.get(message.author.id) != null){
        shift.delete(message.author.id);
    }
    if (!queue) {
    ...
    try{
        ...
        shift.push(message.author.id, queue.songs[parseInt(args[1])].url)
        
        for(var i = 0; i < queue.songs.length; i++){
            if(i != parseInt(args[1])){
                shift.push(message.author.id, queue.songs[i].url)
            }
        }
    } catch(e) {
        console.log(e);
    ...
    if(!(shift.get(message.author.id))){
      const embed = {
    ...
    let array = shift.get(message.author.id)
    shift.delete(message.author.id);
